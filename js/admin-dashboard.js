...
window.baixarEstoquePedidosPendentes = async function baixarEstoquePedidosPendentes() {
    const statusDiv = document.getElementById("sync-status");
    if (!statusDiv) return;

    statusDiv.classList.remove("hidden");
    statusDiv.textContent = "Buscando pedidos com estoque pendente...";

    try {
        const pedidosSnap = await adminDb.collection("pedidos")
            .where("estoque_baixado", "==", false)
            .get();

        if (pedidosSnap.empty) {
            statusDiv.textContent = "Nenhum pedido pendente para baixar estoque.";
            setTimeout(() => statusDiv.classList.add("hidden"), 2600);
            return;
        }

        statusDiv.textContent = `Processando ${pedidosSnap.size} pedido(s)...`;
        ...
        for (const pedidoDoc of pedidosSnap.docs) {
            ...
            await adminDb.runTransaction(async (transaction) => {
                for (const item of (pedido.produtos || [])) {
                    if (!item?.id || !item?.cor?.nome) continue;

                    const produtoRef = adminDb.collection("pecas").doc(item.id);
                    const produtoSnap = await transaction.get(produtoRef);
                    if (!produtoSnap.exists) continue;

                    const prodData = produtoSnap.data() || {};
                    const novasCores = (Array.isArray(prodData.cores) ? prodData.cores : []).map((cor) => {
                        if (cor.nome !== item.cor.nome) return cor;

                        const novaQuantidade = (parseInt(cor.quantidade, 10) || 0) - (parseInt(item.quantity, 10) || 0);
                        return {
                            ...cor,
        ...
        statusDiv.textContent = "Estoque sincronizado com sucesso.";
        refreshOverview();

        if (currentAdminView === "orders") {
            reloadWorkspace("orders");
        }

        if (currentAdminView === "products") {
            reloadWorkspace("products");
        }

        setTimeout(() => statusDiv.classList.add("hidden"), 2600);
    } catch (error) {
        console.error("Erro ao sincronizar estoque:", error);
        statusDiv.textContent = `Falha ao sincronizar: ${error.message || "erro inesperado"}`;
    }
};
